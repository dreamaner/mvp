...
public class LogInterceptor implements Interceptor {
    public static final String TAG = "x-mvp";
    ...
    private void logRequest(Request request) {
        try {
            ...
            XLog.i(TAG, "url : " + url);
            XLog.i(TAG, "method : " + request.method());
            String s = "";
            for (int i = 0, count = headers.size(); i < count; i++) {
                s = "\t" + headers.name(i) + ": " + headers.value(i);
            }
            XLog.i(TAG,s);
            ...
            if (requestBody != null) {
                ...
                if (mediaType != null) {
                    if (isText(mediaType)) {
                        XLog.i(TAG, "params : " + bodyToString(request));
                    } else {
                        XLog.i(TAG, "params : " + " maybe [file part] , too large too print , ignored!");
                    }
                }
            }
        } catch (Exception e) {
            ...
            if (body != null) {
                ...
                if (mediaType != null) {
                    if (isText(mediaType)) {
                        ...
                        XLog.json(Log.INFO, TAG, resp);

                        body = ResponseBody.create(mediaType, resp);
                        return response.newBuilder().body(body).build();
                    } else {
                        XLog.i(TAG, "data : " + " maybe [file part] , too large too print , ignored!");
                    }
                }
            }
