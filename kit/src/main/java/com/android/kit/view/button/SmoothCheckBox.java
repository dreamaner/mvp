package com.android.kit.view.button;
...
import com.android.kit.utils.operate.DimenUtils;
...
public class SmoothCheckBox extends View implements Checkable {
    private static final String KEY_INSTANCE_STATE = "SmoothCheckBox";
    private static final int COLOR_TICK = Color.WHITE;
    private static final int COLOR_UNCHECKED = Color.WHITE;
    private static final int COLOR_CHECKED = Color.parseColor("#00933B");
    ...
    private int min_width = 40;//dp
    private int min_height = 40;//dp
    ...
    private void init(AttributeSet attrs) {
        ...
        int tickColor = ta.getColor(R.styleable.SmoothCheckBox_smoothCheckBox_color_tick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.SmoothCheckBox_smoothCheckBox_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.SmoothCheckBox_smoothCheckBox_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(R.styleable.SmoothCheckBox_smoothCheckBox_color_checked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(R.styleable.SmoothCheckBox_smoothCheckBox_color_unchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_smoothCheckBox_stroke_width, DimenUtils.dpToPxInt(0));
        misRect = ta.getBoolean(R.styleable.SmoothCheckBox_smoothCheckBox_is_rect, misRect);
        ta.recycle();
    ...
    private int measureSize(int measureSpec) {
        int defSize = DimenUtils.dpToPxInt(DEF_DRAW_SIZE);
    ...
    private void drawCenter(Canvas canvas) {
        ...
        if (misRect) {
            ...
            float twmp_width = width * mScaleVal - mStrokeWidth;
            canvas.drawRect(width - twmp_width, width - twmp_width, twmp_width, twmp_width, mPaint);
        } else {
    ...
    private void drawBorder(Canvas canvas) {
        ...
        if (misRect) {
            float width = Math.min(getWidth(), getHeight());
            float twmp_width = width * mFloorScale;
            canvas.drawRect(width - twmp_width, width - twmp_width, twmp_width, twmp_width, mFloorPaint);
        } else {
