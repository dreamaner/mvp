...
import com.android.kit.utils.data.safe.EncryptUtils;
...
public final class AppUtils {
     ...
     * @param mContext 上下文
     */
    public static void init(Context mContext) { //在Application中初始化
        context = mContext;
    ...
    public static void getAppDetailsSettings() {
        getAppDetailsSettings(context.getPackageName());
    }
    ...
    public static boolean cleanAppData(File... dirs) {
        boolean isSuccess = CacheUtils.cleanInternalCache();
        isSuccess &= CacheUtils.cleanInternalDbs();
        isSuccess &= CacheUtils.cleanInternalSP();
        isSuccess &= CacheUtils.cleanInternalFiles();
        isSuccess &= CacheUtils.cleanExternalCache();
        for (File dir : dirs) {
            isSuccess &= CacheUtils.cleanCustomCache(dir);
        }
        return isSuccess;
    }
